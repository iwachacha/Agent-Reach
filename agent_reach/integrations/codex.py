...
def _documentation_summary() -> list[str]:
    return [
        ...
        "Use `--save-dir .agent-reach/shards` plus `agent-reach ledger merge --input .agent-reach/shards --output .agent-reach/evidence.jsonl --json` when per-command shard output is easier than appending into one ledger file.",
        "Use `agent-reach schema collection-result --json` when downstream code needs a contract-testable CollectionResult schema.",
        ...
        "Use `agent-reach ledger summarize --input .agent-reach/evidence.jsonl --json` for neutral evidence health counts, and add `--filter` when the caller wants narrowed counts over one ledger slice.",
...
def _codex_runtime_policy() -> dict[str, Any]:
    ...
    return {
        ...
        "decision_order": [
            ...
            "When communicating channel choices to other tools or agents, use the exact stable channel names from `agent-reach channels --json` and avoid shorthand aliases such as `hatena`.",
    ...
    return {
        ...
        "python_sdk": {
            ...
            "quickstart": [
                ...
                'client.exa_search.search("latest gpt-5.4 release notes", limit=3)',
            ...
            "notes": [
                ...
                "Prefer SDK namespaces that mirror stable channel names such as client.exa_search, client.hatena_bookmark, and client.hacker_news.",
                "Compatibility aliases such as client.exa, client.hatena, and client.hn are SDK-only shortcuts and should not be copied into CLI channel names or client.collect(...) calls.",
            ],
        },
